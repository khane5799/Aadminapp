...
class _JoinRequestsState extends State<JoinRequests> {
  ...
  Widget build(BuildContext context) {
    Future<List<QueryDocumentSnapshot>> fetchAllRequests() async {
    ...
    return DefaultTabController(
      length: 2, // You can make it 3 if you also want "Approved"
      child: Scaffold(
        ...
        appBar: CustomAppBar(
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(text: "Pending"),
              Tab(text: "Rejected"),
              // Tab(text: "Approved"), // Enable if needed
            ],
          ),
        ),
        body: FutureBuilder<List<QueryDocumentSnapshot>>(
          future:
              fetchAllRequests(), // You might adjust this to fetch all memberships instead
          builder: (context, snapshot) {
            ...
            if (!snapshot.hasData || snapshot.data!.isEmpty) {
              return const Center(child: Text("No membership requests found."));
            }

            final allRequests = snapshot.data!;
            final pendingRequests =
                allRequests.where((doc) => doc['status'] == 'pending').toList();
            final rejectedRequests = allRequests
                .where((doc) => doc['status'] == 'rejected')
                .toList();
            // final approvedRequests = allRequests.where((doc) => doc['status'] == 'approved').toList();
  ...
  Widget _buildRequestList(List<QueryDocumentSnapshot> requests) {
    if (requests.isEmpty) {
      return const Center(child: Text("No data in this tab."));
    }
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _infoRow(Icons.markunread_mailbox,
                                    "Postcode: ${data["postcode"] ?? ""}"),
                                _infoRow(
                                    Icons.map, "State: ${data["state"] ?? ""}"),
                                _infoRow(
                                    Icons.work, data["occupation"] ?? "N/A"),
                                _infoRow(Icons.star,
